	...
	void LaunchMissile() {
		if (!LeanTween.isTweening (gameObject)) {
			...
			for (int i = 0; i < 5; i++) {
				...
				GameObject temp = Instantiate (missile, origin, Quaternion.AngleAxis (dir == Direction.RIGHT ? 0f : 180f, Vector3.up)) as GameObject;


				Vector3 tarPos = misslePoint.position + misslePoint.forward * 20f + misslePoint.up * Random.Range (-1f, 1f);
